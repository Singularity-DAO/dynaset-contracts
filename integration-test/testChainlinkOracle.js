...
describe("ChainlinkOracle contract", function () {
  beforeEach(async () => {
      if (!initialized) await init();
  });
  
  describe("Deployment of ChainlinkOracle", () => {

    it("Test deploy ChainlinkOracle", async () => {
      // deploy chainlink oracle
      const oracle = await chainlinkOracle_CF.deploy(chainlink_feedRegistry, USDC, WETH, WBTC);
      console.log("Deployed ChainlinkOracle");
      expect(await oracle.registry())
          .to.be.equal(chainlink_feedRegistry);
      expect(await oracle.USDC())
          .to.be.equal(USDC);
      expect(await oracle.WETH())
          .to.be.equal(WETH);
      expect(await oracle.WBTC())
          .to.be.equal(WBTC);
    });
    ...
    it("Test getPrice(DAI, USDC)", async () => {
        const oracle = await chainlinkOracle_CF.deploy(chainlink_feedRegistry, USDC, WETH, WBTC);
        const results = await oracle.getPrice(DAI, USDC);
        const price = results[0];
        const timestamp = results[1];
        console.log("getPrice(DAI, USDC) = %s (timestamp: %s)", price, timestamp);
    });
    ...
    it("Non-existing chainlink oracle USDT/WBTC should return price and timestamp 0", async () => {
        const oracle = await chainlinkOracle_CF.deploy(chainlink_feedRegistry, USDC, WETH, WBTC);
        const results = await oracle.getPrice(USDT, WBTC);
        const price = results[0];
        const timestamp = results[1];
        console.log("getPrice(USDT, WBTC) = %s (timestamp: %s)", price, timestamp);
        expect(timestamp)
            .to.be.equal(0);
        expect(price)
            .to.be.equal(0);            
