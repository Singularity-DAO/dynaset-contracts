...
const HUNDRED_THOUSAND_DAI = "100000000000000000000000";
const ONE_MILLION_DAI = "1000000000000000000000000";
...
var init = async function() {
      ...
      await useTenMillionUsdWorthOfWhaleFunds(account, "0x5777d92f208679db4b9778590fa3cab3ac9e2168", DAI);
      await useTenMillionUsdWorthOfWhaleFunds(account, "0xf04a5cc80b1e94c69b48f5ee68a08cd2f09a7c3e", WETH);
      
      initialized = true;
}

describe("ForgeV1 contract", function () {
  beforeEach(async () => {
      if (!initialized) await init();
  });
  
  describe("Deployment of Forge", () => {

    it("Test after $1 initialisation, Forge and redeem", async () => {
      // deploy dynaset
      ...
      console.log("Forge deployed and configured: ", forge.address);
       ...
       var dynasetTvlUsdc = await dynasetOracle.dynasetTvlUsdc();
       console.log('dynasetTvlUsdc: %s', (dynasetTvlUsdc/1e6));

       // deposit 1 USDC in forge
       await waitForTx(await forge.deposit(first_forge_id, ONE_USDC, account.address));
       console.log('deposit 1 USDC');

       // query LP balance
       const userInfo = await forge.callStatic.userInfo(first_forge_id, account.address);
       ...
       // Redeem dynaset LP
       var userDynasetShares = await forge.getUserDynasetsOwned(first_forge_id, account.address);
    ...
    it("Test Forge using DAI contribution token and redeem", async () => {
      // deploy dynaset
      const dynasetFactory = await dynasetFactory_CF.deploy(account.address);
      await waitForTx(await dynasetFactory.deployDynaset(account.address, account.address, "Test Dynaset", "dynBTC"));
      console.log("Deployed Dynaset");
      ...
      // deploy forge
      const forge = await forge_CF.deploy(account.address, dynasetAddress, dynasetOracle.address);
      await waitForTx(await forge.createForge(false, DAI, ONE_DAI, HUNDRED_THOUSAND_DAI, ONE_MILLION_DAI));
      await waitForTx(await forge.setDeadline(deadline));
      ...
      // approve spending of tokens by forge
      for (const token of [DAI, WETH, USDC, WBTC]) {
         const tokenContract = new ethers.Contract(token, IERC20_ABI, account);
         await waitForTx(await tokenContract.approve(forge.address, ethers.constants.MaxUint256));
      }
      ...
      console.log("Forge deployed and configured: ", forge.address);
       ...
       // deposit 1 DAI in forge
       await waitForTx(await forge.deposit(first_forge_id, ONE_DAI, account.address));
       ...
       // forge
       var contributorsToMint = 1;
       const expected_minimum_tokens_to_mint = 98;
       await waitForTx(await forge.forgeFunction(first_forge_id, contributorsToMint, expected_minimum_tokens_to_mint, tx_options));
       console.log("Forged");
    ...
    it("Test after $1 million initialisation, Forge and redeem", async () => {
      // deploy dynaset
      const dynasetFactory = await dynasetFactory_CF.deploy(account.address);
      await waitForTx(await dynasetFactory.deployDynaset(account.address, account.address, "Test Dynaset", "dynBTC"));
      console.log("Deployed Dynaset");
      ...
      // calculate 0.5 million USDC worth of WBTC
      const usdc_wbtc_amounts = await uniswapRouter.getAmountsOut(HALF_MILLION_USDC, [USDC, WBTC]);
      const wbtc_amount = usdc_wbtc_amounts[1];
      
      // initialize dynaset
      await waitForTx(await dynasetFactory.initialiseDynaset(dynasetAddress, [USDC, WBTC], [HALF_MILLION_USDC, wbtc_amount], account.address, "500", "500"));
      await waitForTx(await dynasetOracle.updateDynasetTokenPrices());
      console.log("Initialized dynaset");
    ...
    it("Test after $1 million initialisation USDC/DAI, 1 million USDC Forge and redeem without capital slash", async () => {
      // deploy dynaset
      const dynasetFactory = await dynasetFactory_CF.deploy(account.address);
      await waitForTx(await dynasetFactory.deployDynaset(account.address, account.address, "Test Dynaset", "dynDAI"));
      console.log("Deployed Dynaset");
      const first_dynaset_id = 0;
      const dynasetAddress = await dynasetFactory.dynasets(first_dynaset_id);
      console.log("Dynaset: " + dynasetAddress);
      ...
      // approve spending of tokens by dynaset
      for (const token of [WETH, USDC, DAI]) {
         const tokenContract = new ethers.Contract(token, IERC20_ABI, account);
         await waitForTx(await tokenContract.approve(dynasetAddress, ethers.constants.MaxUint256));
      }

      // calculate 0.5 million USDC worth of DAI
      const usdc_dai_amounts = await uniswapRouter.getAmountsOut(HALF_MILLION_USDC, [USDC, DAI]);
      const dai_amount = usdc_dai_amounts[1];
      
      // initialize dynaset
      await waitForTx(await dynasetFactory.initialiseDynaset(dynasetAddress, [USDC, DAI], [HALF_MILLION_USDC, dai_amount], account.address, "500", "500"));
      await waitForTx(await dynasetOracle.updateDynasetTokenPrices());
      console.log("Initialized dynaset");
      
      // deploy forge
      const forge = await forge_CF.deploy(account.address, dynasetAddress, dynasetOracle.address);
      await waitForTx(await forge.createForge(false, USDC, ONE_USDC, ONE_MILLION_USDC, TEN_MILLION_USDC));
      await waitForTx(await forge.setDeadline(deadline));
      ...
      console.log("Forge deployed and configured: ", forge.address);
       ...
       // fast forward blockchain
       var seconds_per_block = 60*60*24 * 90; // skip capital slash period
       await ethers.provider.send("evm_increaseTime", [seconds_per_block]);
       await ethers.provider.send("evm_mine", []);
       ...
       // deposit 1 million USDC in forge
       await waitForTx(await forge.deposit(first_forge_id, ONE_MILLION_USDC, account.address));
       console.log('deposit 1 million USDC');
       ...
       var minimumAmountForged = totalSupply.mul(95).div(100);
       await waitForTx(await forge.forgeFunction(first_forge_id, contributorsToMint, minimumAmountForged, tx_options));
       console.log("Forged");
       ...
       var usdValueAfterFees = await forge.capitalSlash(usdValue, startTime);
       console.log('usdValue after capital slash: %s', (usdValueAfterFees/1e18));
       var minimumAmountOut = usdValueAfterFees.mul(97).div(100).div(1e12); // account for 3% slippage and convert to 6 decimals precision for USDC
       console.log('minimumAmountOut: %s', minimumAmountOut/1e6);
       ...
       console.log('redeemed: %s', redeemed/1e6);
       expect(redeemed).to.be.at.least(minimumAmountOut);
       const ninetySevenPercentOfInput = ethers.constants.One.mul(ONE_MILLION_USDC).mul(97).div(100);
       console.log('97% of input: %s', ninetySevenPercentOfInput/1e6);
       expect(redeemed).to.be.at.least(ninetySevenPercentOfInput);
    ...
    it("Test Forge should fail on redeem when exceeding slippage", async () => {
      // deploy dynaset
      const dynasetFactory = await dynasetFactory_CF.deploy(account.address);
      await waitForTx(await dynasetFactory.deployDynaset(account.address, account.address, "Test Dynaset", "dynBTC"));
      console.log("Deployed Dynaset");
      ...
      // initialize dynaset
      await waitForTx(await dynasetFactory.initialiseDynaset(dynasetAddress, [USDC, WBTC], [ONE_USDC, "42000"], account.address, "500", "500"));
      await waitForTx(await dynasetOracle.updateDynasetTokenPrices());
      console.log("Initialized dynaset");
      ...
      console.log("Forge deployed and configured: ", forge.address);
       ...
       var usdValueAfterFees = await forge.capitalSlash(usdValue, startTime);
       console.log('usdValue after capital slash: %s', (usdValueAfterFees/1e18));
       var minimumAmountOut = usdValueAfterFees.div(1e12); // do not account for slippage, just convert to 6 decimals precision for USDC
       console.log('minimumAmountOut: %s', minimumAmountOut/1e6);
       await expect(forge.redeem(first_forge_id, userDynasetShares, USDC, minimumAmountOut, tx_options))
          .to.be.revertedWith("ERR_MINIMUM_AMOUNT_OUT");
    });

    it("Test Forge, withdraw dynaset LP, deposit dynaset LP and redeem", async () => {
      // deploy dynaset
      const dynasetFactory = await dynasetFactory_CF.deploy(account.address);
      await waitForTx(await dynasetFactory.deployDynaset(account.address, account.address, "Test Dynaset", "dynBTC"));
      console.log("Deployed Dynaset");
      ...
      console.log("Forge deployed and configured: ", forge.address);
       ...
       // Enable dynaset LP withdraw
       await waitForTx(await forge.setlpWithdraw(true, tx_options));      
       // Withdraw dynaset LP
       var userDynasetShares = await forge.getUserDynasetsOwned(first_forge_id, account.address);
       console.log('userDynasetsOwned: %s', (userDynasetShares/1e18));
       await waitForTx(await forge.withdrawOutput(first_forge_id, userDynasetShares , tx_options));
       console.log('Withdrawn user dynaset LP');
       
       // approve dynaset shares to be spend by dynaset
       await waitForTx(await dynaset.approve(forge.address, ethers.constants.MaxUint256));
       // deposit dynaset LP
       await waitForTx(await forge.depositOutput(first_forge_id, userDynasetShares , tx_options));
       console.log('Deposited user dynaset LP');
    ...
    it("Test Forge, withdraw dynaset LP, redeem from wallet", async () => {
      // deploy dynaset
      const dynasetFactory = await dynasetFactory_CF.deploy(account.address);
      await waitForTx(await dynasetFactory.deployDynaset(account.address, account.address, "Test Dynaset", "dynBTC"));
      console.log("Deployed Dynaset");
      ...
      console.log("Forge deployed and configured: ", forge.address);
       ...
       // Redeem dynaset LP
       var userDynasetShares = await dynaset.balanceOf(account.address);
       console.log('userDynasetShares: %s', (userDynasetShares/1e18));
       var perShareUsdcValue = await dynasetOracle.dynasetUsdcValuePerShare();
       console.log('perShareUsdcValue: %s', (perShareUsdcValue/1e18));
       var usdValue = perShareUsdcValue.mul(userDynasetShares).div(DYNASET_PRECISION); // 18 decimals precision
       ...
       const usdcContract = new ethers.Contract(USDC, IERC20_ABI, account);
       const balanceBeforeRedeem = await usdcContract.balanceOf(account.address); 
       await waitForTx(await forge.redeemFromWallet(first_forge_id, userDynasetShares, USDC, minimumAmountOut, tx_options));
       const balanceAfterRedeem = await usdcContract.balanceOf(account.address); 
       console.log('Redeemed dynaset LP');
       const redeemed = balanceAfterRedeem-balanceBeforeRedeem;
       console.log('redeemed: %s', redeemed/1e6);
