...
var init = async function() {
      ...
      await faucet(controller.address, 100000);
      
      // get contract factory
      uniswapv2Oracle_CF = await ethers.getContractFactory("Uniswapv2Oracle");

      // get WETH
      var amountIn = new BigNumber(1000e18).toFixed();
      const wethContract = new ethers.Contract(WETH, WETH_ABI, controller);
      await wethContract.deposit({value:amountIn});
      console.log('Wrap 1000 ETH into WETH');
      ...
      console.log("Swap 1000 ETH for USDC");
      ...
      console.log("Swap 1000 ETH for WBTC");

      initialized = true;
}

describe("Uniswapv2Oracle contract", function () {
  beforeEach(async () => {
      if (!initialized) await init();
  });
  	
  describe("Deployment of Uniswapv2Oracle", () => {

    it("Test deploy Uniswapv2Oracle", async () => {
      // deploy oracle
      const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
      console.log("Deployed Uniswapv2Oracle");
      expect(await oracle.uniswapFactory())
          .to.be.equal(uniswapv2Factory);
      expect(await oracle.observationPeriod())
          .to.be.equal(observation_period);
      expect(await oracle.USDC())
          .to.be.equal(USDC);
      expect(await oracle.WETH())
          .to.be.equal(WETH);
    });
/*
...
*/
    it("Test tokenUsdcValue should revert when no usable price found", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await expect(oracle.tokenUsdcValue(WBTC, ONE_WBTC))
            .to.be.revertedWith("ERR_USABLE_PRICE_NOT_FOUND");
    });

    it("Test tokenUsdcValue(WBTC, 1.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, WBTC);
    ...
    it("Test tokenUsdcValue(USDT, 1.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, USDT);
    ...
    it("Test tokenUsdcValue(WETH, 1.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, WETH);
    ...
    it("Test tokenUsdcValue(MAKER, 1.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, MAKER);
    ...
    it("Test tokenUsdcValue(AAVE, 1.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, AAVE);
    ...
    it("Test tokenUsdcValue(LINK, 1.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, LINK);
    ...
    it("Test tokenUsdcValue(COMP, 1.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, COMP);
    ...
    it("Test tokenUsdcValue(1INCH, 1.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, ONEINCH);
    ...
    it("Test tokenUsdcValue(ELON, 1,000,000,000.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, ELON);
    ...
    it("Test tokenUsdcValue(SHIBA, 1,000,000.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, SHIBA);
    ...
    it("Test tokenUsdcValue(SDAO, 1.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, SDAO);
    ...
    it("Test tokenUsdcValue(AGIX, 1.0)", async () => {
        const oracle = await uniswapv2Oracle_CF.deploy(uniswapv2Factory, observation_period, USDC, WETH);
        await updateOracle(oracle, AGIX);
